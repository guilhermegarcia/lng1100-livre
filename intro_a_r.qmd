...
Importez le fichier `donnees/sampleData.csv`. Donnez-lui le nom `donnees`.
...
`read_csv("donnees/sampleData.csv")`.

Si ce chemin ne fonctionne pas, vérifiez que vous avez ouvert
`LNG1100.Rproj` et que `file.exists("donnees/sampleData.csv")` retourne `TRUE`.
