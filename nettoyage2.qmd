...
Bien que le chapitre précédent contienne un script pour nettoyer nos données, on y a créé des étapes séparément. On pourrait pourtant utiliser le *pipe* pour enchainer toutes nos opérations. Retournez au chapitre précédent et enchainez votre code en utilisant `|>`.
...
On a révisé ci-dessus comment (a) nettoyer un fichier et (b) créer des graphiques. Vous pouvez certainement compléter (a) et (b) dans un même script. Par contre, comme discuté dans le [chapitre 9](nettoyage.qmd), c'est toujours une bonne idée de séparer les différents éléments dans une analyse pour maximiser l'organisation de notre flux de travail.
