...
1. **Je ne peux pas trouver les fichiers de données.** Vérifiez d'abord :
   - que vous travaillez dans le dossier `LNG1100` **décompressé**, et non dans
     `LNG1100.zip`;
   - que vous avez ouvert RStudio en double-cliquant sur `LNG1100.Rproj`;
   - que `file.exists("donnees/villes.csv")` retourne `TRUE`;
   - que le fichier recherché se trouve dans le bon sous-dossier.

   Si le test retourne `FALSE`, fermez RStudio, retrouvez le dossier `LNG1100`
   décompressé, puis ouvrez de nouveau `LNG1100.Rproj`. Consultez les
   [instructions de préparation](fichiers.qmd) pour revoir la structure du
   projet.
2. **L'installation de `tidyverse` ne fonctionne pas.** Vérifiez votre connexion
   ...
   message d'erreur si l'installation échoue encore. En cas de problème majeur,
   utilisez temporairement le
   [plan de secours avec Posit Cloud](fichiers.qmd#sec-cloner); il faudra aussi
   y exécuter `install.packages("tidyverse")` avant `library(tidyverse)`.
3. **Posit Cloud ne fonctionne pas.** Posit Cloud est seulement un plan de
   secours : continuez normalement dans le projet local ouvert avec
   `LNG1100.Rproj`. Vous pouvez vérifier l'état des serveurs sur
...
sur Brio (Évaluation). RStudio installé sur votre ordinateur est la méthode
normale pour le cours; Posit Cloud est seulement un plan de secours. Vous êtes
